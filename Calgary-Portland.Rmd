...
Portland uses Calgary model. Why
...
We are going to load data in that are projected in WGS 84, which is the "web mercator" coordinate system, which is in units of meter.
...
city_boundary_cal_proj <- st_transform(city_boundary_cal, crs(Dis_River_cal))
city_boundary_cal_vect <- vect(city_boundary_cal_proj)
...
# prepare raster data for plots
names(Elevation_cal) <- "Elevation"
Elevation_cal_df <- as.data.frame(Elevation_cal, xy = TRUE, na.rm = TRUE)

names(Dis_River_cal) <- "Dis_River"
Dis_River_cal_terra <- rast(Dis_River_cal)
Dis_River_cal_cropped <- crop(Dis_River_cal_terra, city_boundary_cal_vect)
Dis_River_cal_masked <- mask(Dis_River_cal_cropped, city_boundary_cal_vect)
Dis_River_cal_df <- as.data.frame(Dis_River_cal_masked, xy = TRUE, na.rm = TRUE)

names(Flow_accu_cal) <- "Flow_accu"
Flow_accu_cal_terra <- rast(Flow_accu_cal)
Flow_accu_cal_cropped <- crop(Flow_accu_cal_terra, city_boundary_cal_vect)
Flow_accu_cal_masked <- mask(Flow_accu_cal_cropped, city_boundary_cal_vect)
Flow_accu_cal_df <- as.data.frame(Flow_accu_cal_masked, xy = TRUE, na.rm = TRUE)

names(Slope_cal) <- "Slope"
Slope_cal_terra <- rast(Slope_cal)
Slope_cal_cropped <- crop(Slope_cal_terra, city_boundary_cal_vect)
Slope_cal_masked <- mask(Slope_cal_cropped, city_boundary_cal_vect)
Slope_cal_df <- as.data.frame(Slope_cal_masked, xy = TRUE, na.rm = TRUE)
...
names(Elevation_por) <- "Elevation"
Elevation_por_df <- as.data.frame(Elevation_por, xy = TRUE, na.rm = TRUE)

names(Dis_River_por) <- "Dis_River"
Dis_River_por_df <- as.data.frame(Dis_River_por, xy = TRUE, na.rm = TRUE)

names(Flow_accu_por) <- "Flow_accu"
Flow_accu_por_df <- as.data.frame(Flow_accu_por, xy = TRUE, na.rm = TRUE)

names(Slope_por) <- "Slope"
Slope_por_df <- as.data.frame(Slope_por, xy = TRUE, na.rm = TRUE)
...
Why elevation scale to 0-300: the calgary and portland elevation range is xxx and xxx, about 300 difference. Wht scale elevation
...
```{r Elevation, fig.height=5, fig.width=10}

Elevation_cal_plot <- ggplot(Elevation_cal_df, aes(x = x, y = y, fill = Elevation)) +
  geom_raster() +
...
Elevation_por_plot <- ggplot(Elevation_por_df, aes(x = x, y = y, fill = Elevation)) +
  geom_raster() +
  scale_fill_gradientn(colors = my_colors, na.value = "#e9e9e9",
                      name = "Elevation (m)") +
  coord_equal() +  
...
# Add titles aligned at same level
plot_grid(
  ggdraw() + draw_label("Elevation Map of Calgary", x = 0.5, hjust = 0.5),
  ggdraw() + draw_label("Elevation Map of Portland", x = 0.5, hjust = 0.5),
  Elevation_cal_plot, Elevation_por_plot,
...
```{r Slope, fig.height=5, fig.width=10}

Slope_cal_plot <- ggplot(Slope_cal_df, aes(x = x, y = y, fill = Slope)) +
  geom_raster() +
  scale_fill_gradientn(colors = my_colors, na.value = "#e9e9e9",
                      name = "Slope (%)") +
  coord_equal() +  
...
Slope_por_plot <- ggplot(Slope_por_df, aes(x = x, y = y, fill = Slope)) +
  geom_raster() +
  scale_fill_gradientn(colors = my_colors, na.value = "#e9e9e9",
                      name = "Slope (%)") +
  coord_equal() +  
...
# Add titles aligned at same level
plot_grid(
  ggdraw() + draw_label("Slope Map of Calgary", x = 0.5, hjust = 0.5),
  ggdraw() + draw_label("Slope Map of Portland", x = 0.5, hjust = 0.5),
  Slope_cal_plot, Slope_por_plot,
  ncol = 2, rel_heights = c(0.1, 1),
  align = "v", axis = "lr"
)
...
### 3.1.3 Distance to River


```{r Distance_to_River, fig.height=5, fig.width=10}

Dis_River_cal_plot <- ggplot(Dis_River_cal_df, aes(x = x, y = y, fill = Dis_River)) +
  geom_raster() +
  scale_fill_gradientn(colors = my_colors, na.value = "#e9e9e9",
...
Dis_River_por_plot <- ggplot(Dis_River_por_df, aes(x = x, y = y, fill = Dis_River)) +
  geom_raster() +
  scale_fill_gradientn(colors = my_colors, na.value = "#e9e9e9",
                      name = "Distance to River (m)") +
  coord_equal() +  
...
# Add titles aligned at same level
plot_grid(
  ggdraw() + draw_label("Distance to River Map of Calgary", x = 0.5, hjust = 0.5),
  ggdraw() + draw_label("Distance to River Map of Portland", x = 0.5, hjust = 0.5),
  Dis_River_cal_plot, Dis_River_por_plot,
  ncol = 2, rel_heights = c(0.1, 1),
  align = "v", axis = "lr"
)
...
```{r Flow_Accumulation, fig.height=8, fig.width=10}

Flow_accu_cal_plot <- ggplot(Flow_accu_cal_df, aes(x = x, y = y, fill = sqrt(Flow_accu))) +
  geom_raster() +
  scale_fill_gradientn(colors = c("#9AC3BB", "#033E56", "#033E56","#033E56", "#033E56", "#033E56"), na.value = "#e9e9e9",
                      limits = c(15,  sqrt(max(Flow_accu_cal_df$Flow_accu, na.rm = TRUE))),
                      name = "Distance to River (m)") +
  coord_equal() +  
...
Flow_accu_por_plot <- ggplot(Flow_accu_por_df, aes(x = x, y = y, fill = sqrt(Flow_accu))) +
  geom_raster() +
  scale_fill_gradientn(colors = c("#9AC3BB", "#033E56", "#033E56","#033E56", "#033E56", "#033E56"), na.value = "#e9e9e9",
                      limits = c(15, sqrt(max(Flow_accu_por_df$Flow_accu, na.rm = TRUE))),
                      name = "Distance to River (m)") +
  coord_equal() +  
  theme_void() + theme(legend.position = "none")
...
# Add titles aligned at same level
plot_grid(
  ggdraw() + draw_label("Flow Accumulation Map of Calgary", x = 0.5, hjust = 0.5),
  ggdraw() + draw_label("Flow Accumulation Map of Portland", x = 0.5, hjust = 0.5),
  Flow_accu_cal_plot, Flow_accu_por_plot,
  ncol = 2, rel_heights = c(0.1, 1),
  align = "v", axis = "lr"
)
...
```{r Landcover, fig.height=5, fig.width=10}

Dis_River_cal_plot <- ggplot() + 
    geom_sf(data = city_boundary_cal_proj, fill = "#e9e9e9", col = "grey") +
...
Dis_River_por_plot <- ggplot() + 
    ...
    geom_sf(data = grassland_por, fill = "#D3D477", col = "transparent") + 
    theme_void()

# Add titles aligned at same level
plot_grid(
  ggdraw() + draw_label("Landcover Map of Calgary", x = 0.5, hjust = 0.5),
  ggdraw() + draw_label("Landcover to River Map of Portland", x = 0.5, hjust = 0.5),
  Dis_River_cal_plot, Dis_River_por_plot,
  ncol = 2, rel_heights = c(0.1, 1),
  align = "v", axis = "lr"
)
...
## 3.2 Fishnet Construction

### 3.2.1 Convert original data into fishnet


```{r fishnet_cal, fig.height=5, fig.width=10}

fishnet_cal_flow_plot <- ggplot() + 
  geom_sf(data = fishnet_cal, aes(fill = Flow_accu), col = "transparent", color = NA) +
  scale_fill_gradientn(colors = my_colors, na.value = "#e9e9e9",
                    name = "Elevation (m)") + 
  theme_void()
...
plot_grid(
  ggdraw() + draw_label("Landcover Map of Calgary", x = 0.5, hjust = 0.5),
  ggdraw() + draw_label("Landcover to River Map of Portland", x = 0.5, hjust = 0.5),
  fishnet_cal_flow_plot, Flow_accu_cal_plot,
  ncol = 2, rel_heights = c(0.1, 1),
  align = "v", axis = "lr"
)
