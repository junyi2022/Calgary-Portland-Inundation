...
------------------------------------------------------------------------------------------------
## calgary five variables map visualization as following: 
...
## 1. Load spatial data, including city boundary and variables
```{r load-data, include=FALSE}
# Define raw GitHub URLs for each raster layer
url_elevation   <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/Elevation/cal_elevation.tif"
url_dis_river <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/Distance%20to%20River/dis_river_2.tif"
url_flow_acc    <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/Flow%20Accumulation/flow_acc_cal.tif"
url_slope       <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/Slope/slope_cal.tif"
url_dev   <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/Developed.geojson"
url_for   <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/Forest.geojson"
url_grass <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/Grassland.geojson"
url_city  <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/calgary_city_boundary.geojson"

# Use temporary files to store downloaded raster data
temp_elev   <- tempfile(fileext = ".tif")
temp_river  <- tempfile(fileext = ".tif")
temp_flow   <- tempfile(fileext = ".tif")
temp_slope  <- tempfile(fileext = ".tif")

# Function to download and load a raster from GitHub
load_github_raster <- function(url) {
...
# Load raster layers
Elevation_cal <- load_github_raster(url_elevation)
Dis_River_cal <- load_github_raster(url_dis_river)
Flow_accu_cal <- load_github_raster(url_flow_acc)
Slope_cal     <- load_github_raster(url_slope)

# Load GeoJSON vector layers directly
url_dev   <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/Developed.geojson"
url_for   <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/Forest.geojson"
url_grass <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/Grassland.geojson"
url_city  <- "https://raw.githubusercontent.com/junyi2022/Calgary-Portland-Inundation/main/data/Calgary/shapefile_city_cover/calgary_city_boundary.geojson"

developed_area <- st_read(url_dev)
forest_area    <- st_read(url_for)
grassland_area <- st_read(url_grass)
city_boundary  <- st_read(url_city)
```


## 2. Visualize Variables in Calgary 
```{r}
# -------Plotting Elevation--------
...
city_boundary_proj <- st_transform(city_boundary, crs(Slope_cal))
...
```{r load_Portland_data, warning = FALSE, message = FALSE, results = "hide"}

# load original data

# Load dem TIFF
dem_Portland <- raster("data/Portland/original/dem/dem_mercator_clip.tif")
# Load slope TIFF
disWater_Portland <- raster("data/Portland/original/slope/slope_clip.tif")
# Load distance to water TIFF
disWater_Portland <- raster("data/Portland/original/disWater/dis_water_clip.tif")

# landcover developed
develop_Portland <- st_read("data/Portland/original/developed/develop.shp")
# landcover forest
forest_Portland <- st_read("data/Portland/original/forest/forest.shp")
# landcover grassland
grassland_Portland <- st_read("data/Portland/original/grassland/grassland.shp")
# water
water_Portland <- st_read("data/Portland/original/water/water_merge.geojson")

# load fishnet data

# fishnet value
fishnet_Portland <- st_read("data/Portland/fishnet/fishnet200m_withvalue.shp")
# fishnet boundary
fishnet_boundary_Portland <- st_read("data/Portland/fishnet/fishnetBoundary.shp")
# original Portland boundary
fishnet_boundary_Portland <- st_read("data/Portland/fishnet/portlandBoundary.shp")

```

We are going to load data in that are projected in WGS 84, which is the "web mercator" coordinate system, which is in units of metre.
...
ggplot() +
  geom_sf(data = fishnet_boundary_Portland)+
  geom_sf(data = fishnet_Portland, aes(fill = Flow_accu), color = "transparent") +
  labs(title="Water Area in Portland") +
  theme_void()
